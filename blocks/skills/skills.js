...
export function initCarousel(carousel, desktopVisible = 3) {
  ...
  const PEEK = 56; // px of the next card visible at the right edge, "roller" feel
  ...
  function render() {
    ...
    const gap = 24;
    const peek = slides.length > vc ? PEEK : 0;
    const slideWidth = `calc((100% - ${(vc - 1) * gap}px - ${peek}px) / ${vc})`;
    slides.forEach((slide) => { slide.style.width = slideWidth; });
    // advance by exactly one card + gap per step, using the same slideWidth calc
    track.style.transform = `translateX(calc(-${index} * (${slideWidth} + ${gap}px)))`;
